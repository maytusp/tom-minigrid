...
class TrainConfig:
    '''
    direction_obs: enable or disable an extra direction_observation; agent direction will be already represented in our modified grid obs 
...
def make_states(config: TrainConfig):
    ...
    network = ActorCriticRNN(
        ...
        direction_obs=config.direction_obs,
...
def make_train_funcs(
    env: Environment,
    env_params: EnvParams, 
    config: TrainConfig,
    ):
    # initialization.
    def init_runner_state(rng, train_state, init_hstate):
        rng, _rng = jax.random.split(rng)
        ...
        # Return the "carry" state that we pass between chunks
        return (rng, train_state, timestep, prev_action, prev_reward, init_hstate)
...
def train(config: TrainConfig):
    ...
    # 2. Replicate for Devices
    rng = jax.random.split(rng, num=jax.local_device_count())
    train_state = replicate(train_state, jax.local_devices())
    init_hstate = replicate(init_hstate, jax.local_devices())

    # 3. Create Compiled Functions
    print("Compiling...")
    init_runner_fn, train_chunk_fn = make_train_funcs(env, env_params, config)
    
    # PMAP the functions
    init_runner_fn = jax.pmap(init_runner_fn, axis_name="devices")
    train_chunk_fn = jax.pmap(train_chunk_fn, axis_name="devices")
    
    # Initialize Runner State on GPU
    runner_state = init_runner_fn(rng, train_state, init_hstate)
    print("Compilation Complete.")

    # 4. Main Training Loop
    num_chunks = config.num_updates // config.log_frequency
    total_transitions = 0
    
    print(f"Starting training for {config.num_updates} updates ({num_chunks} logging chunks)...")
    t_start = time.time()

    for chunk_idx in range(num_chunks):
        # Run one chunk of updates (e.g., 10 updates)
        runner_state, metrics_chunk = train_chunk_fn(runner_state)
        
        # Block until ready to keep Python loop in sync with GPU
        metrics_chunk = jax.tree_util.tree_map(lambda x: x.block_until_ready(), metrics_chunk)
        
        # Unreplicate and process metrics
        # metrics_chunk shape: (num_devices, log_freq, ...)
        metrics = unreplicate(metrics_chunk)
        
        # Loop through the chunk to log each step
        for i in range(config.log_frequency):
            total_transitions += config.num_steps * config.num_envs_per_device * jax.local_device_count()
            
            # Extract metrics for this specific update step
            step_metrics = jtu.tree_map(lambda x: x[i].item(), metrics)
            step_metrics["transitions"] = total_transitions
            step_metrics["progress"] = (chunk_idx * config.log_frequency + i) / config.num_updates
        ...
        if current_update % config.save_every == 0:
            ...
            with open(save_path, "wb") as f:
                f.write(to_bytes(current_params))
            print(f"Saved checkpoint: {save_path}")

    elapsed_time = time.time() - t_start
    print(f"Training finished in {elapsed_time:.2f}s")

    # --- SAVE FINAL MODEL ---
    final_params = unreplicate(runner_state[1].params)
    with open(checkpoint_path, "wb") as f:
        f.write(to_bytes(final_params))
